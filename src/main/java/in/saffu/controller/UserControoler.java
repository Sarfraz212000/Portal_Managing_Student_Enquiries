...
import in.saffu.constants.AppConstant;
...
public class UserControoler {
	...
	public String handelSingup(SingUpForm form, Model model) {
		...
		if (status) {
			...
			model.addAttribute(AppConstant.ERROR_MSG, "choose unique email");
		}
		model.addAttribute("user", new SingUpForm());
		return "signup";
	}
	...
	public String unlockPage(@RequestParam("email") String email, Model model) {
		...
		form.setEmail(email);
		model.addAttribute(AppConstant.UNLOCK_MSG, form);
		return AppConstant.UNLOCK_MSG;
	}
	...
	public String unlockAcc(@ModelAttribute("unlock") UnlockForm unlock, Model model) {
		...
		if (unlock.getNewPwd().equals(unlock.getConfirmPwd())) {
			...
			if (status) {
				...
				model.addAttribute(AppConstant.ERROR_MSG, "temppwd incorrect ,check your email");
			}
		} else {
			model.addAttribute(AppConstant.ERROR_MSG, "please check new and confirm password is corect ya not?");
		}
		return AppConstant.UNLOCK_MSG;
	}
	...
	public String login(@ModelAttribute("loginForm") LoginForm form, Model model) {
		...
		model.addAttribute(AppConstant.ERROR_MSG, login);
	...
	public String forgotpwd(@RequestParam("email") String email, Model model) {
		...
		if (status) {
			...
			model.addAttribute(AppConstant.ERROR_MSG, "inavalid Email");
		}
